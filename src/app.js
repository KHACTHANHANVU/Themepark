...
http.createServer(function(req, res) {
    ...
    } else if (req.url == '/sales_report.html') {
        console.log('here2')
        fs.readFile(path.join(__dirname, '..', 'public', 'sales_report.html'), function (err, file) {
            if (err) {console.log(__dirname); throw err;}
            res.writeHead(200, {'Content-Type': 'text/html'});
            res.write(file);
            res.end();
        });
    } else if (req.url == '/repair%20log.html') {
        console.log('here2')
        fs.readFile(path.join(__dirname, '..', 'public', 'repair log.html'), function (err, file) {
            if (err) {console.log(__dirname); throw err;}
            res.writeHead(200, {'Content-Type': 'text/html'});
            res.write(file);
            res.end();
